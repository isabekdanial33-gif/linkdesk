...
public final class NativeRtc {
 ...
 public synchronized void start(String id){if(pc!=null)return;room=id;
  ...
  control.registerObserver(new DataChannel.Observer(){public void onBufferedAmountChange(long previous){}public void onStateChange(){emit("channel",control.state().toString().toLowerCase(Locale.ROOT));}public void onMessage(DataChannel.Buffer b){if(b.binary||b.data.remaining()>600000)return;byte[] bytes=new byte[b.data.remaining()];b.data.get(bytes);emit("message",new String(bytes,java.nio.charset.StandardCharsets.UTF_8));}});
  DataChannel.Init ai=new DataChannel.Init();ai.ordered=false;ai.maxRetransmits=0;audio=pc.createDataChannel("audio-pcm-48000",ai);
