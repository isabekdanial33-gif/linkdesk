...
PBKDF2-SHA256 separates authentication and AES-GCM keys. AAD binds directions and frame context. Monotonic receive sequences reject replay. Signaling and relay payloads are encrypted; WebRTC uses an ordered data channel. JPEG frames are sent in 14KB chunks at about 5fps directly, or ~1fps through the fallback. Desktop supports mouse, keyboard, Unicode text, multi-monitor selection; Android uses explicit MediaProjection consent + AccessibilityService. No shell execution endpoint, persistence, unattended control or background startup.
